...
async function CreateTranslationJSON() {
	...
	JSONTrans = await ProcessXlfFile('Select source auto-generated xlf file', JSONTrans);
...
function WriteJSONTrans(linetext, JSONTrans, LastSourceText) {
	const reRemoveLabels = /s*(<target>|<source>)(.*)(<\/target>|<\/source>)/gm;
...
async function WriteElementToEdit(element,WSEdit,CurrDoc,lastLine)
{
	...
	lastLine = lastLine + 1;
	await WSEdit.insert(CurrDoc.uri, new vscode.Position(lastLine, 0), 'Target=>'+element.source.trim());
	await vscode.commands.executeCommand('editor.action.insertLineAfter');	
	return(lastLine);
}
